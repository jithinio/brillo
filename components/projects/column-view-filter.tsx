...
function SortableColumnItem({ column, onVisibilityToggle }: SortableColumnItemProps) {
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
                        "flex items-center justify-between p-3 border-b border-border bg-background hover:bg-muted/50 transition-colors",
                isDragging && "opacity-50 shadow-lg border border-border rounded-md"
      )}
    >
      ...
      <PopoverTrigger asChild>
        <Button 
          ...
          className={cn(
            ...
            hasHiddenColumns && "border-muted-foreground bg-muted text-muted-foreground",
            className
          )}
        >
