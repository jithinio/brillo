...
export default function CancelledProjectsPage() {
  ...
  // Calculate metrics for cancelled projects
  const calculateMetrics = () => {
    const totalProjects = projects.length
    const totalBudget = projects.reduce((sum, project) => sum + (project.budget || 0), 0)
    const totalReceived = projects.reduce((sum, project) => sum + (project.received || 0), 0)
    const totalPending = projects.reduce((sum, project) => sum + (project.pending || 0), 0)
  ...
  return (
    <div className="h-full flex flex-col">
      <PageHeader title="Cancelled Projects" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 w-full mb-6">
          <div className="rounded-lg border bg-transparent text-card-foreground p-3 h-16 flex items-center">
            <div className="flex items-center gap-2 w-full">
              <h3 className="text-xs font-medium text-muted-foreground leading-none whitespace-nowrap">Cancelled Projects</h3>
              <div className="text-lg font-semibold text-gray-600 ml-auto">{metrics.totalProjects}</div>
            </div>
          </div>
          <div className="rounded-lg border bg-transparent text-card-foreground p-3 h-16 flex items-center">
            <div className="flex items-center gap-2 w-full">
